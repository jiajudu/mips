...
#define vgabase 0x90000000
static row = 0;
static column = 0;
static char VGAdata[2400];
...
static void vga_putc(int c){
    ...
    if(c == '\n'){
        ...
        if(row > 0 || column > 0){
            if(column > 0){
                column --;
                VGAdata[row * 80 + column] = ' ';
                *(((char*)vgabase) + row * 80 + column) = ' ';
            }else{
                row --;
                column = 79;
                VGAdata[row * 80 + column] = ' ';
        ...
        VGAdata[row * 80 + column] = ch;
        *(((char*)vgabase) + row * 80 + column) = ch;
        column ++;
        if(column == 80){
            column = 0;
            row ++;
    ...
    if(row == 30){
        for(i = 0; i < 29; i++){
            for(j = 0; j < 80; j++){
                VGAdata[i * 80 + j] = VGAdata[(i + 1) * 80 + j];
                *(((char*)vgabase) + i * 80 + j) = VGAdata[(i + 1) * 80 + j];
            }
        }
        for(j = 0; j < 80; j++){
            VGAdata[29 * 80 + j] = ' ';
            *(((char*)vgabase) + 29 * 80 + j) = ' ';
        }
        row = 29;
        column = 0;
    }
